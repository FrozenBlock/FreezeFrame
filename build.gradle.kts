...
val githubRelease by tasks.registering {
    val fabricJar = mainJarTask(project(":ff-fabric"))
    val neoforgeJar = mainJarTask(project(":ff-neoforge"))
    dependsOn(fabricJar, neoforgeJar)

    val token = env["GITHUB_TOKEN"]
    val repository = mod.repository.get()
    val tag = project(":ff-fabric").version.toString()
    val releaseTitle = "Freeze Frame $tag"
