...
public class FilmScreen extends Screen {
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		...
		if (wheel != 0) {
			...
			if (currentIndex != updatedIndex) {
				...
				Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(FFSounds.FILM_ROLL, (float) (0.8F + (Math.random() * 0.4F))));
			}
		}
	...
	private void deleteSelectedPhotograph() {
		...
		Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(FFSounds.FILM_TEAR, (float) (0.8F + (Math.random() * 0.25F))));
		this.onSelectedPhotographChanged();
	}
	...
	private void reloadFromHeldFilm() {
		...
		if (!filmStack.isEmpty()) {
			final FilmContents contents = filmStack.getOrDefault(FFDataComponents.FILM_CONTENTS, FilmContents.EMPTY);
