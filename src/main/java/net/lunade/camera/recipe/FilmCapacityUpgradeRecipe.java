...
package net.lunade.camera.recipe;
...
public class FilmCapacityUpgradeRecipe extends CustomRecipe {
	...
	public boolean matches(CraftingInput input, Level level) {
		...
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 3; x++) {
				...
				if (x == 1 && y == 1) {
					if (!stack.is(CameraPortItems.FILM)) return false;
	...
	public ItemStack assemble(CraftingInput input) {
		...
		if (!sourceFilm.is(CameraPortItems.FILM)) return ItemStack.EMPTY;
		...
		upgradedFilm.set(CameraPortDataComponents.FILM_MAX_PHOTOGRAPHS, upgradedMax);
	...
	public RecipeSerializer<? extends CustomRecipe> getSerializer() {
		return CameraPortRecipeSerializers.FILM_CAPACITY_UPGRADE;
	}
}
