...
public class CameraItem extends SpawnEggItem {
	...
	public InteractionResult use(Level level, Player player, InteractionHand hand) {
		...
		if (hand != InteractionHand.OFF_HAND) {
			player.playSound(FFSounds.CAMERA_SCOPE_START.get(), 1F, 0.85F + (player.level().getRandom().nextFloat() * 0.5F));
	...
	private void stopUsing(LivingEntity entity) {
		entity.playSound(FFSounds.CAMERA_SCOPE_END.get(), 1F, 0.8F + (entity.level().getRandom().nextFloat() * 0.2F));
	}
	...
	private static Fraction getInsertedFilmWeightSafe(CameraContents contents) {
		...
		final FilmContents filmContents = Objects.requireNonNullElse(filmStack.get(FFDataComponents.FILM_CONTENTS.get()), FilmContents.EMPTY);
		return FilmItem.getWeightSafe(filmContents, filmStack);
	}

	public static float getFullnessDisplay(ItemStack stack) {
		final CameraContents contents = stack.getOrDefault(FFDataComponents.CAMERA_CONTENTS.get(), CameraContents.EMPTY);
	...
	public boolean overrideStackedOnOther(ItemStack self, Slot slot, ClickAction clickAction, Player player) {
		...
		if (clickAction == ClickAction.PRIMARY && !other.isEmpty()) {
			...
			self.set(FFDataComponents.CAMERA_CONTENTS.get(), contents.toImmutable());
	...
	public static int getSelectedItemIndex(ItemStack stack) {
		return stack.getOrDefault(FFDataComponents.CAMERA_CONTENTS.get(), CameraContents.EMPTY).getSelectedItemIndex();
	}

	@Nullable
	public static ItemStackTemplate getSelectedItem(ItemStack stack) {
		return stack.getOrDefault(FFDataComponents.CAMERA_CONTENTS.get(), CameraContents.EMPTY).getSelectedItem();
	}
	...
	public static boolean isCapableOfTakingPhotos(ItemStack stack) {
		return stack.getOrDefault(FFDataComponents.CAMERA_CONTENTS.get(), CameraContents.EMPTY).hasSpaceForPhotograph();
	}

	public static FilmFilter getFilterForNextPhotograph(ItemStack cameraStack) {
		return getFilterForNextPhotograph(cameraStack.getOrDefault(FFDataComponents.CAMERA_CONTENTS.get(), CameraContents.EMPTY));
	}
	...
	public static FilmFilter getFilterForNextPhotograph(CameraContents contents) {
		for (ItemStackTemplate filmStack : contents.items()) {
			final FilmContents filmContents = filmStack.get(FFDataComponents.FILM_CONTENTS.get());
			if (filmContents == null) continue;
			if (FilmItem.getWeightSafe(filmContents, filmStack).compareTo(Fraction.ONE) < 0) {
				return filmStack.getOrDefault(FFDataComponents.FILM_FILTER.get(), FilmFilter.EMPTY);
			}
		}
	...
	public Optional<TooltipComponent> getTooltipImage(ItemStack camera) {
		...
		return !display.shows(FFDataComponents.CAMERA_CONTENTS.get())
			? Optional.empty()
			: Optional.ofNullable(camera.get(FFDataComponents.CAMERA_CONTENTS.get())).map(CameraTooltip::new);
	}
	...
	private static void playSnapFailSound(Entity entity, boolean playsSeparateClientSound) {
		...
		if (playsSeparateClientSound && entity instanceof ServerPlayer serverPlayer) {
			FrozenLibSoundPackets.createAndSendLocalPlayerSound(
				serverPlayer,
				FFSounds.CAMERA_SNAP_FAIL.asHolder(),
				1F,
				0.8F + entity.level().getRandom().nextFloat() * 0.4F
			);
		}
	...
	public static void playRemoveOneSound(Entity entity) {
		entity.playSound(
			FFSounds.CAMERA_REMOVE_ONE.get(),
			0.8F,
			0.8F + entity.level().getRandom().nextFloat() * 0.4F
		);
	}
	...
	private static void playInsertFailSound(Entity entity) {
		entity.playSound(
			FFSounds.CAMERA_INSERT_FAIL.get(),
			0.8F,
			0.95F + entity.level().getRandom().nextFloat() * 0.1F
		);
	}
