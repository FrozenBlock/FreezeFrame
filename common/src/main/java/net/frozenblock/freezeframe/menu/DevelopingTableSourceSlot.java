...
public class DevelopingTableSourceSlot extends Slot {
	...
	public static boolean isValidFilmForPrinting(ItemStack stack) {
		return stack.is(FFItems.FILM.get());
	}

	public static boolean isValidBookSource(ItemStack stack) {
		return stack.is(Items.WRITABLE_BOOK) || stack.is(Items.WRITTEN_BOOK) && stack.has(FFDataComponents.BOOK_PAGE_PHOTOGRAPHS.get());
	}

	public static boolean isValidPhotographForCopying(ItemStack stack) {
		return stack.is(FFItems.PHOTOGRAPH.get())
			&& stack.has(FFDataComponents.PHOTOGRAPH.get())
			&& stack.get(FFDataComponents.PHOTOGRAPH.get()).canCopy();
	}
}
