...
public final class ScopeZoomHelper {
	public static final ScopeZoomConfig CAMERA_DEFAULTS = new ScopeZoomConfig(
		...
		1F,
		false,
		Optional.of(FFSounds.CAMERA_ZOOM_INCREASE.asHolder()),
		Optional.of(FFSounds.CAMERA_ZOOM_DECREASE.asHolder())
	);
	public static final ScopeZoomConfig SPYGLASS_DEFAULTS = new ScopeZoomConfig(
		...
		10F,
		true,
		Optional.of(FFSounds.SPYGLASS_ZOOM_INCREASE.asHolder()),
		Optional.of(FFSounds.SPYGLASS_ZOOM_DECREASE.asHolder())
	);
	...
	public static float getStoredZoom(ItemStack stack) {
		...
		return stack.getOrDefault(FFDataComponents.SCOPE_ZOOM.get(), config.defaultZoom());
	}
	...
	private static ScopeZoomConfig getZoomConfig(ItemStack stack) {
		final ScopeZoomConfig explicitConfig = stack.get(FFDataComponents.SCOPE_ZOOM_CONFIG.get());
