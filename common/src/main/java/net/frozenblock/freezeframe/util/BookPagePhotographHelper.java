...
public final class BookPagePhotographHelper {
	...
	public static ItemStack getPhoto(ItemStack book, int pageIndex) {
		...
		final BookPagePhotographs pages = book.get(FFDataComponents.BOOK_PAGE_PHOTOGRAPHS.get());
		if (pages == null) return ItemStack.EMPTY;

		return pages.photographs().stream()
			.filter(page -> page.pageIndex() == pageIndex)
			.findFirst()
			.filter(page -> page.photograph().is(FFItems.PHOTOGRAPH.get()))
	...
	public static void setPhoto(ItemStack book, int pageIndex, ItemStack photoStack) {
		...
		if (entries.isEmpty()) {
			book.remove(FFDataComponents.BOOK_PAGE_PHOTOGRAPHS.get());
		} else {
			book.set(FFDataComponents.BOOK_PAGE_PHOTOGRAPHS.get(), new BookPagePhotographs(entries));
		}
	}
	...
	public static BookPagePhotographs incrementPhotoGenerationsForBookCopy(BookPagePhotographs photographs) {
		...
		for (BookPagePhotographs.PagePhotograph pagePhotograph : photographs.photographs()) {
			...
			if (stack.is(FFItems.PHOTOGRAPH.get())) {
				final Photograph photograph = stack.get(FFDataComponents.PHOTOGRAPH.get());
				if (photograph != null) stack.set(FFDataComponents.PHOTOGRAPH.get(), photograph.asBookCopy());
			}
