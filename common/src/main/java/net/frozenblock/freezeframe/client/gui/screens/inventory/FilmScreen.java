...
public class FilmScreen extends Screen {
	private static final Identifier TEXTURE = FFConstants.id("textures/gui/container/film.png");
	private static final Identifier SCROLLER = FFConstants.id("container/film/scroller");
	private static final Identifier SCROLLER_DISABLED = FFConstants.id("container/film/scroller_disabled");
	private static final Identifier FILM_PHOTOGRAPH_BLOCKER = FFConstants.id("container/film/film_photograph_blocker");
	private static final Identifier FILM_PHOTOGRAPH_HIGHLIGHT = FFConstants.id("container/film/film_photograph_highlight");
	private static final Identifier FILM_PHOTOGRAPH_SAVE = FFConstants.id("container/film/film_photograph_save");
	private static final Identifier FILM_PHOTOGRAPH_DELETE = FFConstants.id("container/film/film_photograph_delete");
	private static final Identifier TEXT_FIELD = FFConstants.id("container/film/text_field");
	private static final Identifier TEXT_FIELD_DISABLED = FFConstants.id("container/film/text_field_disabled");
	private static final Identifier DELETE_BUTTON = FFConstants.id("container/film/delete_button");
	private static final Identifier DELETE_BUTTON_HOVER = FFConstants.id("container/film/delete_button_hover");
	...
	public FilmScreen(Player owner, InteractionHand hand) {
		super(Component.translatable("screen.freezeframe.film.title"));
		this.owner = owner;
		this.hand = hand;
	...
	protected void init() {
		...
		this.nameEditBox = new EditBox(
			...
			NAME_TEXT_WIDTH,
			NAME_TEXT_HEIGHT,
			Component.translatable("screen.freezeframe.film.name")
		);
		...
		this.saveButton = this.addRenderableWidget(Button.builder(Component.translatable("screen.freezeframe.film.save"), button -> this.saveChangesAndClose())
			.bounds(this.leftPos + SAVE_BUTTON_X, this.topPos + ACTION_BUTTON_Y, ACTION_BUTTON_WIDTH, ACTION_BUTTON_HEIGHT)
			.build());
		this.cancelButton = this.addRenderableWidget(Button.builder(Component.translatable("screen.freezeframe.film.cancel"), button -> this.cancelChangesAndClose())
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		...
		if (wheel != 0) {
			...
			if (currentIndex != updatedIndex) {
				...
				Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(FFSounds.FILM_ROLL.get(), (float) (0.8F + (Math.random() * 0.4F))));
			}
		}
	...
	private void saveChangesAndClose() {
		...
		ClientNetworkingHelper.sendToServer(new SaveFilmChangesPacket(this.hand, this.toFilmContents(), !this.modifiedPhotographIds.isEmpty()));
	...
	private void deleteSelectedPhotograph() {
		...
		Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(FFSounds.FILM_TEAR.get(), (float) (0.8F + (Math.random() * 0.25F))));
		this.onSelectedPhotographChanged();
	}
	...
	private void reloadFromHeldFilm() {
		...
		if (!filmStack.isEmpty()) {
			final FilmContents contents = filmStack.getOrDefault(FFDataComponents.FILM_CONTENTS.get(), FilmContents.EMPTY);
