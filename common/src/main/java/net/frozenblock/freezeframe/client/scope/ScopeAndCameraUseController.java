...
public final class ScopeAndCameraUseController {
	...
	private static void tick(Minecraft minecraft) {
		...
		final boolean isScopeConfigDifferent = !previousScopeItem.getOrDefault(FFDataComponents.SCOPE_ZOOM_CONFIG.get(), ScopeZoomConfig.EMPTY)
			.equals(scopeItem.getOrDefault(FFDataComponents.SCOPE_ZOOM_CONFIG.get(), ScopeZoomConfig.EMPTY));
		...
		if (attackDown && !wasAttackDown && minecraft.gameMode != null && holdingCamera) {
			if (ScopeItemHelper.isPlayerUsingCamera(player)) {
				ClientNetworkingHelper.sendToServer(new ChangeScopeZoomPacket(player.getUsedItemHand(), ScopeZoomManager.getZoom()));
				minecraft.gameMode.useItem(player, player.getUsedItemHand());
			} else {
				ClientNetworkingHelper.sendToServer(new QuickCameraPhotographPacket());
			}
		}
