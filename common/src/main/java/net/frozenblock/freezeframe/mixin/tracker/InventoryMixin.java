...
public class InventoryMixin {
	...
	public void freezeFrame$onItemAbsorbedFromCreativePlayer(ItemStack instance, int count, Operation<Void> original) {
		if (!this.player.level().isClientSide() && FFConfig.PHOTOGRAPH_TRACKER.get()) {
			if (count != 0) {
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onItemAbsorbedFromCreativePlayer - Inventory: Non-zero setCount called!", true);
			} else {
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onItemAbsorbedFromCreativePlayer - Inventory", true);
			}
	...
	public void freezeFrame$onDamagedItemAbsorbedFromCreativePlayer(ItemStack instance, int count, Operation<Void> original) {
		if (!this.player.level().isClientSide() && FFConfig.PHOTOGRAPH_TRACKER.get()) {
			if (count != 0) {
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onDamagedItemAbsorbedFromCreativePlayer - Inventory: Non-zero setCount called!", true);
			} else {
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onDamagedItemAbsorbedFromCreativePlayer - Inventory", true);
			}
			PhotographTracker.incrementOnItemStackSizeChange(this.player.level(), instance.copy(), -(instance.getCount() - count));
		}
		original.call(instance, count);
