...
public abstract class AbstractContainerMenuMixin {
		...
		if (this.quickcraftType == QUICKCRAFT_TYPE_CLONE && FFConfig.PHOTOGRAPH_TRACKER.get()) {
			...
			if (increaseCount > 0) {
				if (player.level().isClientSide()) {
					ClientNetworkingHelper.sendToServer(new ChangeItemStackSizePacket(itemStack.copy(), increaseCount));
				} else {
					PhotographTracker.incrementOnItemStackSizeChange(player.level(), itemStack.copy(), increaseCount);
				}
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onDragCloneQuickCraft - AbstractContainerMenu", true);
			}

			if (!adjustedForFirst.get()) {
				if (player.level().isClientSide()) {
					ClientNetworkingHelper.sendToServer(ChangeItemStackSizePacket.itemStackDeleted(source.copy()));
				} else {
					PhotographTracker.incrementOnItemStackDeletion(player.level(), source.copy());
				}
				if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onDragCloneQuickCraft (Adjust Against Carried) - AbstractContainerMenu", true);
	...
	public ItemStack freezeframe$onSafeClone(ItemStack original) {
		if (FFConfig.PHOTOGRAPH_TRACKER.get()) {
			ClientNetworkingHelper.sendToServer(ChangeItemStackSizePacket.itemStackCloned(original.copy()));
			if (FFConstants.UNSTABLE_LOGGING) FFConstants.log("onSafeClone - AbstractContainerMenu", true);
		}
		return original;
	}
}
